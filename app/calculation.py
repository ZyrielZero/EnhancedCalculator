"""The Calculation value object: one performed calculation, with its result."""
...
@dataclass
class Calculation:
    """A single calculation: its operation name, operands, result, and time.

    The result is computed the moment the object is built, so a Calculation
    always holds a finished answer rather than a pending one.
    """
    ...
    def __post_init__(self):
        """Compute and store the result as soon as the operands are set."""
        self.result = self.calculate()

    def calculate(self) -> Decimal:
        """Look up the operation by name and run it on the operands."""
        operations = {
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize to a plain dict of strings for CSV storage."""
        return {
    ...
    def from_dict(data: Dict[str, Any]) -> 'Calculation':
        """Rebuild a Calculation from a stored dict, re-running the math."""
        try:
            calc = Calculation(
    ...
    def format_result(self, precision: int = 10) -> str:
        """Render the result trimmed to `precision` decimals, no trailing zeros."""
        try:
